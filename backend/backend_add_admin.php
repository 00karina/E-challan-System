...
if (isset($_POST['create'])) {
	...
	if ($row > 0) {
		...
		header("Location: ../html/addadmin.php?error=Username already taken");
		exit();
	} else {
		//validates names
		if (preg_match("/^[a-zA-z]+$/", $first_name) && preg_match("/^[a-zA-z]+$/", $last_name)) {
			...
			if (preg_match("/^[9]+[7-8]+\d{8}$/", $phone_no)) {
				...
				$query_add = "INSERT INTO user(first_name, middle_name, last_name, username, password, address, phone_no, role, gender) VALUES ('$first_name','$middle_name','$last_name','$username','$password','$address','$phone_no','$role','$gender')";
				//inserts data
				if (mysqli_query($conn, $query_add)) {
					header("Location: ../html/addadmin.php?success=User added successfully");
				} else {
					header("Location: ../html/addadmin.php?error=Error while adding user");
				}
			} else {
